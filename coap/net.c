...
coap_pdu_t *
simple_post_response(coap_context_t *context, coap_pdu_t *request) {
  coap_pdu_t *resp;
  coap_opt_iterator_t opt_iter;
  size_t len, wkc_len;
  unsigned char buf[2];
  ...
  if (!coap_add_token(resp, request->hdr->token_length, request->hdr->token)) {
    debug("simple_post_response: cannot add token\n");
    goto error;
  }
  /* Check if there is sufficient space to add Content-Format option 
   * and data. We do this before adding the Content-Format option to
   ...
   * content. */
  if (resp->max_size <= (size_t)resp->length + 3) {
    debug("simple_post_response: insufficient storage space\n");
    goto error;
  }
...
handle_request(coap_context_t *context, coap_queue_t *node) {      
  ...
  if (!resource) {
    ...
    case COAP_REQUEST_POST:
	  printf("COAP POST REQUEST\n");
	  response = simple_post_response(context, node->pdu);

	  break;
	  
	case COAP_REQUEST_PUT:
	  printf("COAP PUT REQUEST");
	  break;
	  
	 case COAP_REQUEST_DELETE:
	  printf("COAP DELETE REQUEST");
	  break;
	  
	 case COAP_REQUEST_FETCH:
	  printf("COAP FETCH REQUEST");
	  break;
	  
	 case COAP_REQUEST_PATCH:
	  printf("COAP PATCH REQUEST");
	  break; 
